...
//   // useful when saving the description in local storage?
  ...
  var userInput = $(this).siblings("textarea").val();
   var dailyTasks =  $(this).parent().attr("id");
  console.log(dailyTasks);
  localStorage.setItem(dailyTasks,userInput);
 ...
 function load(event){
  ...
  for(var i = 0; i < ids.length; i++){
   ...
   var localStorages = ids.children("textarea").val(localStorage.getItem[i]);
    console.log(localStorages)
    localStorage.getItem(localStorages);
  }
 }
 load();
...
//   // current hour in 24-hour time?
  currentHour = dayjs().format("H")//military time;
